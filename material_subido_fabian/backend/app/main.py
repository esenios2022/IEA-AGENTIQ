...
def startup():
    ...
    print(f"✅ Available LLM providers: {LLMFactory.get_available_providers()}")
...
def get_available_llms():
    ...
    return {
        "available": available,
        "default": next((k for k, v in available.items() if v), "ollama")
    }
...
def run_agent(
    ...
    # Crear ejecución
    execution = Execution(agent_id=agent_id, status="running")
    db.add(execution)
    db.commit()
